def reduce_value(target_list, index):
...
def shot_for_the_win(target_sequence):
    ...
    while True:
        ...
        if 0 <= index < len(target_sequence) and target_sequence[index] != -1:
            ...
            reduce_value(target_sequence, index)

    convert_target_value_to_string = [str(num) for num in target_sequence]
    print(f"Shot targets: {count_of_shot_targets} -> {' '.join(convert_target_value_to_string)}")
